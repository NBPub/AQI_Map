{% for id in sensor_data['id'] %}
  ...
  {{ '}' }}).addTo(map).bindPopup("{{ sensor_data['name'][loop.index0] }} | <b>{{ sensor_data['aqi'][loop.index0]}}</b><br><a target='_blank' href='{{ base_map_link }}{{sensor_data['lat'][loop.index0] }}/{{ sensor_data['lng'][loop.index0] }}'>Purple Air {{ id }}</a>");
{% endfor %}